...
| 3 | *"**o papel vem do KIND, não da posição**"* (e o *"ruling (b)"* de que `collect.dart:238` se diz corolário) | `collect.dart:165`, `type_table.dart:253` | **Não achei artefato.** Nem em ADR, nem em spec, nem na constituição. É load-bearing: decide `superclass-not-a-class` × `trait-expected`. |
| 4 | *"**Trait é FOLHA** … nenhuma aresta sai de um trait"* | `collect.dart:198` | **Não achei artefato.** Cria o diagnóstico **`trait-supertype`** — um erro que o usuário leva, fundado só num carimbo de data. |
