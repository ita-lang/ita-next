...
| [0019](ADR-0019-corpo-do-init.md) | **O corpo do `init`** — a restrição a `self.campo = e` é restrição-para-caber (o Kernel tem `LocalInitializer`); levantá-la é RESTAURAÇÃO. 5 rulings + 3 bordas na mesa | **Accepted** *(2026-07-29: R1 = corte **IMPLÍCITO** no último `self.x =`; R2/R3/R4 + bordas implementados)* | 2026-07-29 |
| [0020](ADR-0020-abi-de-chamada-label-e-fn-como-valor.md) | **ABI de chamada, `_` e `fn` como valor** — 5 opções; **teorema**: marcador de dupla ABI ou viaja (e quebra a transitividade do `==`) ou não viaja (e a promessa se cumpre só no arquivo da decl). Art. II aponta Elixir (`&f`) | **Accepted** *(2026-07-29: 1 = **(E) `&dobro`** · 3 = label segue **OPCIONAL** (a lista de isenções seria maior que o ganho) · 2 = `_` **não entra agora**)* | 2026-07-29 |
