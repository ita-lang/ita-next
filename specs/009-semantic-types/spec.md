...
3. **Contextualidade genuína**: closures (`{ $0*2 }` — o tipo de `$0` vem do esperado), `nil` (§4.6), `.variant`, **literais de coleção (n ≥ 0)**. Nenhum deles sintetiza; todos **checam**. É o modo `check` se sustentando sozinho.
   > **Emenda de 2026-08-31 — era *"coleções vazias (`[]`)"*.** Esta linha desalinhava da §4.3, que é a regra e cujo exemplo é **não-vazio** (`[Cachorro()]` contra `List<Animal>`). O desalinhamento não era inócuo: a leitura *"só o vazio é contextual"* se consolidou na spec 012 e o não-vazio ficou sem regra em fase nenhuma — `[10, 20, 30]` dava `cannot-infer` em contexto nenhum, e **não havia como construir uma `List` com conteúdo em Itá**. O vazio é o **caso-base** (n = 0) da mesma regra, não uma forma à parte. ⚠️ Os dois **fundamentos**, porém, seguem distintos: o vazio não sintetiza por **vacuidade** (6.5.1, definicional), o não-vazio por **política** (§4.9 — o mesmo escudo do `.variant`); usar "vacuidade" para o não-vazio seria trocar o escudo forte pelo fraco, como esta spec já alerta sobre o `.variant`.
