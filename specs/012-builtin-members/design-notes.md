...
- **A (🟠) — ✅ PARCIALMENTE RESOLVIDO (2026-08-31), e a leitura original errava a metade que importa.** ~~Literal de coleção nu (`[1,2,3]`) não sintetiza (é fatia C, spec 010) ⟹ `[1,2,3].length` dá `cannot-infer`. **Dependência da fatia C**, não bug da 012.~~ O achado vale para o literal **nu** (sem esperado), e ali segue verdadeiro — mas por **política da 009 §4.3**, não por fatia faltando (a 010 §4.1 funda o vazio na *vacuidade*, argumento que não se estende a quem tem elementos). Aplicá-lo a **todo** literal foi o erro: sob `let` anotado / parâmetro tipado / retorno anotado o esperado EXISTE, e a 009 §4.3 manda o literal checar (*"`[Cachorro()]` contra esperado `List<Animal>` desce elemento a elemento"*). Medido em 2026-08-31: as três formas davam `cannot-infer` ⟹ **impossível construir `List`/`Map` com conteúdo**, e os CA1–CA3/CA9/CA10 eram **inexecutáveis**, não "dependentes". Corrigido na [errata da 010 §4.1](../010-contextual-typing/spec.md); a metade sem esperado é **ruling do dono** (A × B), com o custo declarado: literal não liga type-var.
- **B (🟠) — `xs + []` dá cannot-infer. SEGUE ABERTO, com cura conhecida e fora do escopo por decisão, não por limite.** O `_binary` SINTETIZA os dois operandos (`check.dart:2173-2174`), então nenhum literal à direita de um operador vê esperado — nem o vazio, nem o não-vazio. Não é a limitação de A (aquela caiu). **Também não é limite da forma bidirecional**, e chamá-la assim seria restrição-para-caber: o 6.5.1 trata operador como **aplicação** (*"A regra (6.8) pode ser adaptada para E1 + E2 visualizando-a como uma **aplicação da função add(E1, E2)**"*), o que faz do direito um argumento, e o 5.2.4(b) autoriza derivar seu esperado do tipo **sintetizado do esquerdo** (L-atribuído, um walk). Cura: `_check(n.right, l)` **se e só se** `_isCheckingOnly(n.right)` — uma linha, critério sintático já implementado, e **zero** diagnóstico atual muda (`1 + "a"` segue `no-operator-for-types`). ⚠️ Ela destrava `xs + [1]`, **não** o CA3 (`([1,2] + [3]).length`), cujo operando esquerdo é literal nu — esse depende do ruling A × B da errata. Travado por teste no grupo *"errata 010 §4.1"* (o corte declarado, 5 sítios).
...
- **E (🟡) — ✅ FECHADO (2026-08-31), junto com a errata da 010.** ~~`xs[[]]` tipa `Int` em silêncio (o ramo coleção-vazia do `_check` não valida `expected`). Furo do `_check`, anterior à 012.~~ Medido antes: `fn m(xs: List<Int>) -> Int => xs[[]]` compilava com **exit 0**, e o `ListExpr` ficava registrado na nº1 com tipo `Int`. Ficou escondido enquanto a F7 dava ICE em todo literal; no dia em que ela emitir, seria `typeArgument` de `ListLiteral` tirado de um `Int`. **Quem estende o ramo herda o furo** — por isso foi fechado no mesmo commit, e não depois: o ramo agora cobra a forma do esperado (kind + aridade dos args) antes de aceitar. Catraca: dois casos no grupo *"errata 010 §4.1"* (`let x: Int = []` e `xs[[]]`).
