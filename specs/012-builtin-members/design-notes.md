...
- **B (🟠) — `xs + []` dá cannot-infer. SEGUE ABERTO, e agora se sabe por quê.** O `_binary` SINTETIZA o operando direito (`r = _synth(right)`), então nenhum literal à direita de um operador vê esperado — nem o vazio, nem o não-vazio. Não é a limitação de A (aquela caiu): é o **operador não propagar contexto**. A cura seria o `+` checar o direito contra o tipo do esquerdo; fica deferida, e o teste que a exercitaria vive no mesmo grupo da errata.
