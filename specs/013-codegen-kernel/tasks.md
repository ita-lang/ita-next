...
**Fronteiras restantes:** só `ice_cmp_on_string`. Antes: Novos ICEs honestos que a fatia criou, cada um uma fatia futura: `fn-generic` (∀), `fn-async` (§12-2), `param-default`, `call-toplevel-<T>` (construtor de struct/class), `call-LocalRes` (valor-função/closure).
