...
- [ ] **Gate 1 — F6 completa:** exaustividade de `match` implementada — ver [`014/tasks.md` LT-F6b](../014-flow-check/tasks.md). A F7 confia na nº8 `flowFacts` (definite-return, para o *throw* defensivo de fim-de-corpo — spec 014 §7) **e** na exaustividade (para emitir `match` sound).
- [ ] **Gate 2 — pin do SDK:** `make pin` (`tools/pin-dart.sh`) — materializa o Dart 3.12.2 pinado **e o vendor `pkg/kernel`** (`third_party/dart/` **ainda não existe** no `ita-next` — as citações `arquivo:linha` da spec foram transcritas do oracle, mesma tag). O gate **CA12** (`verifyComponent`) exige o vendor presente. Confirmar `dart-sdk.pin` (formato Kernel 130, sha256).

---

## LT-F7a — Passes de saneamento pós-construção + re-enquadrar §7.1 `[🔴2 + 🟠5]`
